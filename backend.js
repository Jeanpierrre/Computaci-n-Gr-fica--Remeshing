...
app.post('/api/print', (req, res) => {
  ...
  exec(comando, (err, stdout, stderr) => {
    if (err) {
      console.error('Error al ejecutar final_proj.exe:', stderr);
      return res.status(500).json({ error: 'Error al ejecutar final_proj.exe' });
    }
    ...
    try { fs.unlinkSync(inputPath); } catch (e) { console.warn('input bloqueado'); }
    try { fs.unlinkSync(outputPath); } catch (e) { console.warn('output bloqueado'); }
...
app.post('/api/voronoi', (req, res) => {
  ...
  const comando = `quadratic_remesh.exe ${inputPath} ${outputPath} ${req.body.targetVertices || 20000}`;
  console.log('Ejecutando comando:', comando);

  exec(comando, (err, stdout, stderr) => {
    if (err) {
      console.error('Error al ejecutar quadratic_remesh.exe:', stderr);
      return res.status(500).json({ error: 'Error al ejecutar quadratic_remesh.exe' });
    }
